...
$format =~ m/^(refGene|knownGene|ensGene|genericGene|tab|simple|ccdsGene)$/ or pod2usage ("Error in argument: the --format argument can be only 'refGene', 'knownGene', 'ensGene', 'genericGene', 'tab' or 'simple'");
...
for my $curchr (sort keys %$allchr) {
	...
	for my $i (0 .. @{$allregion->{$curchr}}-1) {
		...
		#print STDERR "Name:$name\tStart:$start\tEnd:$end\texonpos:$exonpos\n";
		...
		if (defined $seq) {
			if (defined $seqlen{$name}) {
				...
				#print STDERR"length($name):$seqlen{$name}\n";
				#if($name eq 'NM_025259'){
				#	die;
				#}
			}			
			#if ($tabout) {
...
for my $i (0 .. @$sorted_region-1) {
	...
	if ($tabout) {
		...
		print ">$name ", $discordlen{$name}?"Warning: $name occur more than once in file with discordant length. ":" ", $badorf{$name, $exonpos}?"Warning: this coding transcript does not have correct ORF annotation. ":" ", 
			"Comment: The length of this sequence (leftmost exon at $exonpos) is $seqlen{$name}, based on regions speficied in $regionfile and ", $seqdir?"sequence files stored at $seqdir.\n":"the sequence file $seqfile.\n",
...
sub readAllRegion {
	...
	while (<REGION>) {
		...
		if ($format eq 'knownGene' or $format eq 'refGene' or $format eq 'ensGene' or $format eq 'genericGene' or $format eq 'ccdsGene') {
